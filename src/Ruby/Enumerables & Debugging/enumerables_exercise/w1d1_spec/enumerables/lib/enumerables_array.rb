...
def subwords(word, dictionary)
  ...
  words = []
  all_substrings.each { |word| words << word if dictionary.include?(word) && !words.include?(word)}
  words
end
...
def concatenate(strings)  # V.V. Important
  concat = ""
  strings.inject(concat) { |acc, el| acc << el }
  concat
end